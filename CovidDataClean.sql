...
select location, row_date, total_case, new_Case, population, (total_Case/population*100) as AFFECTED_PERCENTAGE 
from c_case 
where location='Canada';

//affected percentage - round off to 3 decimal
select location, row_date, total_case, new_Case, population, (Round((total_Case/population*100),4)) as AFFECTED_PERCENTAGE 
from c_case 
where location='Canada' 
order by row_date;

//death percentage
select location, row_date, total_case, total_Death, population, (Round((total_death/population*100),4)) as DEATH_PERCENTAGE 
from c_case 
where location='Canada' 
order by row_date;
...
//Select queries I expect to see null records
  ...
  select count(location), location from (select distinct median_age, location 
                                          from c_case group by location, median_Age) 
  group by location 
  having count(location)>1;

  //check on population_density field
  select count(location), location 
  from (select distinct population_density, location 
                                         from c_case 
